...
\draw[thick,->] (0,0) -- (6,0) node[anchor=west]{Renda (Y)};
\draw[thick,->] (0,0) -- (0,6) node[anchor=south]{Juros (i)};
\foreach \x in {0,1,2,3,4,5}
   \draw (\x cm,1pt) -- (\x cm,-1pt) node[anchor=north] {$\x$};
\foreach \y in {1,2,3,4,5}
    \draw (1pt,\y cm) -- (-1pt,\y cm) node[anchor=east] {$\y$};
\draw[thick, green] (1,5) -- (5,1) node[anchor=east]{IS};
\draw[thick, red] (1,1) -- (5,5)node[anchor=east]{LM};
\filldraw[black]  (3,3) circle (2pt) node[anchor=north]{Equilíbrio};
\end{tikzpicture}
\caption{Modelo de Mendell-Fleming}
\end{figure}

\begin{figure}
\centering
\begin{tikzpicture}
\draw[thick,->] (0,0) -- (6,0) node[anchor=west]{Câmbio (E)};
\draw[thick,->] (0,0) -- (0,6) node[anchor=south]{Juros (i)};
\foreach \x in {0,1,2,3,4,5}
   \draw (\x cm,1pt) -- (\x cm,-1pt) node[anchor=north] {$\x$};
\foreach \y in {1,2,3,4,5}
    \draw (1pt,\y cm) -- (-1pt,\y cm) node[anchor=east] {$\y$};
\draw[thick, blue] (1,5) -- (5,1) node[anchor=east] {Câmbio};
\filldraw[black]  (3,3) circle (2pt) node[anchor=north]{Equilíbrio};
\end{tikzpicture}
\caption{Câmbio no modelo de Mendell-Fleming}
\end{figure}


\subsection{Políticas macroeconômicas}
Levando em conta políticas expansionistas e câmbio flexível:
...
Se pensarmos no sentido de câmbio fixo, perceberemos que a política fiscal é \emph{plena}, já que o banco central não possui autoridade para utilizar da política monetária. E se a taxa de juros é alterada por outros motivos, o banco central intervém para levá-la ao patamar inicial:

\[ G \uparrow \rightarrow Y \uparrow \rightarrow i\uparrow \]

Como $i = i^*$ e a taxa estrangeira não se alterou, o banco central intervém de modo expansionista para diminuir a taxa de juros. Isso empurra a IS mais ainda e aumenta a renda mais do que o impacto inicial:
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}
      ...
      \addplot[color=blue, domain=0:10]{x};\addlegendentry{BP}
        \end{axis}
    \end{tikzpicture}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}
      ...
      \addplot[color=blue,domain=0:5]{3};\addlegendentry{BP 1}
      \addplot[color=orange] coordinates {(3,0) (3,5)};\addlegendentry{BP 2}
        \end{axis}
    \end{tikzpicture}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}
      ...
      \addplot[color=blue,domain=0:5]{x};\addlegendentry{BP}
      \addplot[color=green, mark=*] coordinates{(3,4)}; \addlegendentry{S}
      \addplot[color=red, mark=*] coordinates{(3,1)}; \addlegendentry{D}
      \addplot[color=black, mark=*] coordinates{(3,3)}; \addlegendentry{E}
        \end{axis}
    \end{tikzpicture}
